...
const Testimonilas = () => {
  const halfTestmonial = Math.floor(testimonials.length / 2);
  return (
    <section className="relative z-2 py-24 md:py-28 lg:py-40">
      <div className="container block lg:flex">
        <div className="testimonials_head-res relative z-2 mr-20 flex-300">
          <p className="caption mb-5 max-md:mb-2.5">Wall of Love</p>
          <h3 className="h3 max-md:h5 test-p4">Words from our fans</h3>
        </div>
        <div
      ...
      relative -my-12 -mr-3 flex items-start max-lg:static max-md:block"
        >
          <div className="testimonials_group-after flex-500">
            {testimonials.slice(0, halfTestmonial).map((testimonial) => (
              <TestimonialItem
                key={testimonial.id}
                item={testimonial}
          ...
          <div className="flex-50">
            {testimonials.slice(halfTestmonial).map((testimonial) => (
              <TestimonialItem
                key={testimonial.id}
                item={testimonial}
